...
//echo $html_content;
	...
	public static function alexa_rank($url) 
	{
		...
		return (is_array($p)) ? ((count($p)>1)?$p[2]:NULL) : NULL;
	}
	...
	public static function alexa_links($url) 
	{
		...
		return (is_array($l)) ? ((count($l)>=1)?$l[1]:NULL) : NULL;
	}
	...
	public static function google_links($url) 
	{
		...
		return (is_array($l)) ? ((count($l)>1)?$l[2]:NULL) :  NULL;
	}
	...
	public static function domain_age($domain) 
	{
		...
		if (is_array($p)) 
		{
			return (count($p)>0)?strtotime($p[1]):NULL;
		}
		return NULL;
	}
...
//var_dump($data);
        ...
        $r = (isset($data->shares))?$data->shares:NULL;
